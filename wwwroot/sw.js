const CACHE_NAME = 'navlog-ipad-shell-v9';
const APP_SHELL = [
  './',
  './index.html?v=20260711-9',
  './offline.html',
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then(response => {
      ...
      return caches.match('./index.html?v=20260711-9').then(cached => cached || caches.match('./offline.html'));
    }).catch(() => caches.match('./index.html?v=20260711-9')).catch(() => caches.match('./offline.html')));
    return;
  }
